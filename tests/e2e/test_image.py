...
def test_find_images():
    ...
    with ClientV1(token=token, pdf_data=str(pdf_path), base_url=base_url, read_timeout=30.0) as client:
        ...
        images_page0 = client.find_images(Position.from_page_index(0))
...
def test_move_image():
    ...
    with ClientV1(token=token, pdf_data=str(pdf_path), base_url=base_url, read_timeout=30.0) as client:
        ...
        assert client.move(image_ref, Position.on_page_coordinates(11, 50.1, 100.0)) is True
...
def test_find_image_by_position():
    base_url, token, pdf_path = _require_env_and_fixture('ObviouslyAwesome.pdf')
    with ClientV1(token=token, pdf_data=str(pdf_path), base_url=base_url, read_timeout=30.0) as client:
        images = client.find_images(Position.on_page_coordinates(11, 0, 0))
        assert len(images) == 0
        images = client.find_images(Position.on_page_coordinates(11, 55, 310))
        assert len(images) == 1
...
def test_add_image():
    ...
    with ClientV1(token=token, pdf_data=str(pdf_path), base_url=base_url, read_timeout=30.0) as client:
        ...
        pos = Position.on_page_coordinates(6, 50.1, 98.0)
        ...
        images_page6 = client.find_images(Position.from_page_index(6))
