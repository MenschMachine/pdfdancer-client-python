...
class PDFAssertions(object):
    ...
    def assert_path_exists_at(self, x: float, y: float, page=0, tolerance: float = 5.0) -> 'PDFAssertions':
        """Assert that at least one path exists at the specified coordinates."""
        paths = self.pdf.page(page).select_paths_at(x, y, tolerance)
        assert len(paths) > 0, f"Expected at least 1 path at ({x}, {y}) on page {page}, but found none"
        return self

    def assert_path_count_at(self, count: int, x: float, y: float, page=0, tolerance: float = 5.0) -> 'PDFAssertions':
        """Assert exact number of paths at the specified coordinates."""
        paths = self.pdf.page(page).select_paths_at(x, y, tolerance)
        assert len(paths) == count, f"Expected {count} paths at ({x}, {y}) but got {len(paths)}"
        return self

    def assert_path_has_id(self, internal_id: str, x: float, y: float, page=0, tolerance: float = 5.0) -> 'PDFAssertions':
        """Assert that a path with specific ID exists at the coordinates."""
        paths = self.pdf.page(page).select_paths_at(x, y, tolerance)
        path_ids = [p.internal_id for p in paths]
        assert internal_id in path_ids, f"Expected path {internal_id} at ({x}, {y}), but found: {path_ids}"
        return self

    def assert_path_bounding_box(self, x: float, y: float, width: float, height: float,
                                  page=0, tolerance: float = 5.0, epsilon: float = 1.0) -> 'PDFAssertions':
        ...
        assert bbox.x == pytest.approx(x, abs=epsilon), f"Bounding box x {bbox.x} != {x}"
        assert bbox.y == pytest.approx(y, abs=epsilon), f"Bounding box y {bbox.y} != {y}"
        assert bbox.width == pytest.approx(width, abs=epsilon), f"Bounding box width {bbox.width} != {width}"
        assert bbox.height == pytest.approx(height, abs=epsilon), f"Bounding box height {bbox.height} != {height}"
        return self
    ...
    def assert_path_segment_count(self, expected_count: int, x: float, y: float,
                                   page=0, tolerance: float = 5.0) -> 'PDFAssertions':
    ...
    def assert_line_segment_points(self, segment: Line, p0: Point, p1: Point,
                                    epsilon: float = 0.1) -> 'PDFAssertions':
        ...
        assert segment.get_p0().x == pytest.approx(p0.x, abs=epsilon), \
            f"Line p0.x {segment.get_p0().x} != {p0.x}"
        assert segment.get_p0().y == pytest.approx(p0.y, abs=epsilon), \
            f"Line p0.y {segment.get_p0().y} != {p0.y}"

        assert segment.get_p1().x == pytest.approx(p1.x, abs=epsilon), \
            f"Line p1.x {segment.get_p1().x} != {p1.x}"
        assert segment.get_p1().y == pytest.approx(p1.y, abs=epsilon), \
            f"Line p1.y {segment.get_p1().y} != {p1.y}"
        return self

    def assert_bezier_segment_points(self, segment: Bezier, p0: Point, p1: Point,
                                      p2: Point, p3: Point, epsilon: float = 0.1) -> 'PDFAssertions':
        ...
        for actual, expected, name in [
            ...
            assert actual.x == pytest.approx(expected.x, abs=epsilon), \
                f"Bezier {name}.x {actual.x} != {expected.x}"
            assert actual.y == pytest.approx(expected.y, abs=epsilon), \
                f"Bezier {name}.y {actual.y} != {expected.y}"
        return self

    def assert_segment_stroke_color(self, segment: PathSegment, color: Color) -> 'PDFAssertions':
        ...
        assert stroke_color.r == color.r, f"Stroke color R {stroke_color.r} != {color.r}"
        assert stroke_color.g == color.g, f"Stroke color G {stroke_color.g} != {color.g}"
        assert stroke_color.b == color.b, f"Stroke color B {stroke_color.b} != {color.b}"
        assert stroke_color.a == color.a, f"Stroke color A {stroke_color.a} != {color.a}"
        return self

    def assert_segment_fill_color(self, segment: PathSegment, color: Color) -> 'PDFAssertions':
    ...
    def assert_segment_stroke_width(self, segment: PathSegment, width: float,
                                     epsilon: float = 0.1) -> 'PDFAssertions':
        ...
        assert stroke_width == pytest.approx(width, abs=epsilon), \
            f"Stroke width {stroke_width} != {width}"
        return self

    def assert_segment_has_dash_pattern(self, segment: PathSegment,
        ...
        for i, (actual, expected) in enumerate(zip(actual_array, dash_array)):
            assert actual == pytest.approx(expected, abs=epsilon), \
                f"Dash array[{i}] {actual} != {expected}"

        actual_phase = segment.get_dash_phase()
        if actual_phase is not None:
            assert actual_phase == pytest.approx(dash_phase, abs=epsilon), \
    ...
    def assert_segment_type(self, segment: PathSegment, expected_type: type) -> 'PDFAssertions':
        """Assert that a segment is of a specific type (Line or Bezier)."""
        assert isinstance(segment, expected_type), \
            f"Expected segment type {expected_type.__name__} but got {type(segment).__name__}"
        return self

    def assert_path_has_even_odd_fill(self, x: float, y: float, expected: bool,
                                       page=0, tolerance: float = 5.0) -> 'PDFAssertions':
        ...
        if hasattr(path, 'even_odd_fill') and path.even_odd_fill is not None:
            assert path.even_odd_fill == expected, \
                f"Expected even_odd_fill={expected} but got {path.even_odd_fill}"
        else:
            pytest.skip("Path even-odd fill inspection requires full path data from API")
        return self
