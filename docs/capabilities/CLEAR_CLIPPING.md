...
- Added `PDFDancer.clear_path_group_clipping(page_number, group_id)` and internal `_clear_path_group_clipping(...)` in `src/pdfdancer/pdfdancer_v1.py`. These validate a 1-based client-facing page number and non-empty group ID, then send `PUT /pdf/path-group/clipping/clear` using 0-based `pageIndex` in the request payload (`page_number - 1`).
- Added a temporary compatibility retry in `_clear_path_group_clipping(...)` for deployments that still validate this endpoint with 1-based `pageNumber`. In that specific case, the client retries once with `{"pageNumber": page_number, "groupId": ...}` and caches that compatibility mode for subsequent calls in the same client instance.
- Added convenience methods in `src/pdfdancer/types.py`: `PDFObjectBase.clear_clipping()` forwards the current object's `object_ref`, and `PathGroupObject.clear_clipping()` forwards `self._page_index + 1` into the 1-based `clear_path_group_clipping(page_number, ...)` client API.
